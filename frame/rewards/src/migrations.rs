...
use crate::{Mints, RewardChanges, BalanceOf, Config};
...
impl frame_support::traits::StorageInstance for __CurveV0 {
	fn pallet_prefix() -> &'static str { "Rewards" }
	const STORAGE_PREFIX: &'static str = "Curve";
}

#[allow(type_alias_bounds)]
type CurveV0<T: Config> = frame_support::storage::types::StorageValue<__CurveV0, Vec<CurvePointV0<T::BlockNumber, BalanceOf<T>>>>;
...
#[allow(type_alias_bounds)]
type AdditionalRewardsV0<T: Config> = frame_support::storage::types::StorageValue<__AdditionalRewardsV0, Vec<(T::AccountId, BalanceOf<T>)>>;
