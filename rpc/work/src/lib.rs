...
impl<Block, C, L, Proof> RpcWork<Block, C, L, Proof> where
	Block: BlockT<Hash = H256>,
	...
	Proof: Send + 'static,
{
	pub fn new(client: Arc<C>, worker: Arc<Mutex<MiningWorker<Block, RandomXAlgorithm<C>, C, L, Proof>>>) -> Self {
		Self { client, worker }
	}
}

impl<Block, C, L, Proof> RpcWorkApi for RpcWork<Block, C, L, Proof> where
	Block: BlockT<Hash = H256>,
	C: HeaderBackend<Block> + AuxStore + sp_api::ProvideRuntimeApi<Block> + 'static,
	C::Api: DifficultyApi<Block, Difficulty> + AlgorithmApi<Block>,
	L: sc_consensus::JustificationSyncLink<Block> + 'static,
...
{
	fn get_compute(&self) -> Result<Compute, jsonrpc_core::Error> {
		let metadata = self.worker.lock().metadata().ok_or(internal("metadata does not exist"))?;

		let key_hash = kulupu_pow::key_hash(self.client.as_ref(), &BlockId::Hash(metadata.best_hash)).map_err(|_| internal("fetch key hash failed"))?;
